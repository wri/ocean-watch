...
export const fetchConfigFile = () => {
  ...
  if (process.env.NODE_ENV === 'development')
    return Promise.resolve(
      OceanWatchConfigFile[
        process.env.NEXT_PUBLIC_API_ENV === 'staging' ? 'staging' : 'production'
      ],
    );

  return axios
    .get(
      'https://raw.githubusercontent.com/wri/ocean-watch/main/public/static/data/ocean-watch.json',
    )
    .then(
      ({ data }) => data[process.env.NEXT_PUBLIC_API_ENV === 'staging' ? 'staging' : 'production'],
    );
};

export const fetchOceanWatchAreas = () =>
  axios
    .get('https://wri-rw.carto.com:443/api/v2/sql', {
      params: {
    ...
    .then(({ data }) =>
      data.rows.map(({ iso, name, geostore }) => ({
        iso,
        name,
        geostore,
      })),
