...
class LayerPopup extends PureComponent {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    const { data: popupData, latlng } = this.props;
  ...
  fetchDataUrl(layer, latlng) {
    ...
    this.setState(
      ...
      () => {
        axios
          .get(replace(interactionConfig.config.url, latlng))
          .then(({ data }) => {
  ...
  render() {
    ...
    const { layers, layersInteraction, layersInteractionSelected } = data;
        ...
        <div className="popup-content">
          {!isEmpty(interaction.data || interactionState.data) &&
            output.map((outputItem) => {
              ...
              return (
                <div className="popup-field" key={outputItem.property || outputItem.column}>
                  <div className="field-title">{outputItem.property || outputItem.column}</div>
